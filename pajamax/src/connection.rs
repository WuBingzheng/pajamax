...
pub fn serve2_with_config<A>(
    srvs: Vec<Arc<dyn PajamaxService + Send + Sync + 'static>>,
    addr: A,
    config: Config,
) -> std::io::Result<()>
where
...
pub fn handle2(
    mut srvs: Vec<Arc<dyn PajamaxService + Send + Sync + 'static>>,
    mut c: TcpStream,
    config: Config,
) -> Result<(), Error> {
